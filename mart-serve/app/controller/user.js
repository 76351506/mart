 ...
 * @Last Modified time: 2023-07-01 18:04:07
 */
"use strict";
...
class UserController extends Controller {
   ...
   */
  async list() {
    const result = await this.ctx.service.user.list(this.ctx.query);
    if (result.length) {
      this.ctx.body = {
        code: 1,
        msg: "获取成功!",
  ...
  async modileLogin() {
    ...
    if (
      new Date().getTime() - this.ctx.session[mobile + "MOBILEVerifyTime"] >
      60000
    )
    ...
    if (
      this.ctx.session[mobile + "MOBILEVerify"] !=
      this.ctx.request.body.verifyCode
    ) {
      return (this.ctx.body = {
        code: 0,
        msg: "验证码错误",
    ...
    const result = await this.ctx.service.user.modileLogin(
  ...
  async modifyPwdByMobile() {
    ...
    const result = await this.ctx.service.user.modifyPwdByMobile(
      this.ctx.request.body
    );
  ...
  async modifyPwdByEmail() {
    ...
    if (
      this.ctx.session[email + "EMAILverify"] !=
      this.ctx.request.body.verifyCode
    ) {
      return (this.ctx.body = {
        code: 0,
    ...
    if (
      new Date().getTime() - this.ctx.session[email + "EMAILverifyTime"] >
      60000
    )
      return (this.ctx.body = {
        code: 0,
    ...
    const result = await this.ctx.service.user.modifyPwdByEmail(
      this.ctx.request.body
    );
    if (result.affectedRows) {
      return (this.ctx.body = {
  ...
  async registry() {
    ...
    if (hasUser) {
      ...
      const res = await this.ctx.service.user.setuserinfo(
        this.ctx.request.body,
        uid
      );
  ...
  async updataUserInfo() {
    const result = await this.ctx.service.user.updataUserInfo(
      this.ctx.request.body
    );
    if (result.affectedRows) {
      this.ctx.status = 200;
