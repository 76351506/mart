 ...
 * @Last Modified time: 2023-07-05 10:24:23
 */
"use strict";
...
const {
  PRODUCT_DEFAULT_SORTNAME,
  PRODUCT_DEFAULT_SORTTYPE,
} = require("../config");
...
class ProductService extends Service {
  ...
  async sort({ t_type }) {
    const $sql = `select * from sort where t_type=${t_type}`;
    return await this.ctx.app.mysql.query($sql);
  }
  async list({ pagesize, pagecount }) {
    const $sql =
      pagesize && pagecount
        ? `select * from product limit ${(pagecount - 1) * pagesize},${
            pagesize * pagecount
          }`
        : `SELECT * FROM product`;
    return await this.ctx.app.mysql.query($sql);
  ...
  async add({
    ...
    const $sql =
      "insert into product (pid,pname, imgUrl, sales, original_price, sale_price, `mode`, s_type, t_type, carousel, `desc` ) values (?,?,?,?,?,?,?,?,?,?,?)";
    return await this.ctx.app.mysql.query($sql, $params);
  }
  ...
  async typeDel({ tid }) {
    ...
    let res = await this.ctx.app.mysql.query($sql1, [tid]);
  ...
  async sortDel({ sid }) {
    const $sql = `delete from sort where sid=?`;
    const $params = [sid];
    return await this.ctx.app.mysql.query($sql, $params);
  }
  async edit({
    ...
    const $sql =
      "UPDATE `mart`.`product` SET `pname`=?, `imgUrl`=?, `sales`=?, `original_price`=?, `sale_price`=?, `mode`=?, `s_type`=?, `t_type`=?, `carousel`=?, `desc`=? WHERE  `pid`=?";
    const $params = [
      pname,
      imgUrl,
      sales,
