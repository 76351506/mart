 ...
 * @Last Modified time: 2023-07-01 17:45:54
 */
"use strict";
...
class UserService extends Service {
  async list({ pagesize, pagecount }) {
    const $sql =
      pagesize && pagecount
        ? `select * from user limit ${(pagecount - 1) * pagesize},${
            pagesize * pagecount
          }`
        : `SELECT * FROM user`;
  ...
  async registry({ username, password, mobile }, uid) {
    ...
    const $sql =
      "insert into login (uid,username,password,mobile,identity_id) values (?,?,?,?,?)";
    return await this.ctx.app.mysql.query($sql, $params);
  }
  ...
  async cutAvatar(avatar, uid) {
    ...
    this.ctx.app.mysql.query(
      "update comment set uphoto=? where uid=?",
      $params
    );
  ...
  async selectAvatar(imgUrl) {
    return await this.ctx.app.mysql.query(
      "select * from avatar where imgUrl=?",
      [imgUrl]
    );
  }
