 ...
 * @Last Modified time: 2023-06-26 17:38:44
 */
"use strict";
...
class CarouselService extends Service {
  async add({ tit, start_time, end_time, type, img }) {
    const cid = idCreator('13').slice(0, 20);
    type = type || 0;
  ...
  async edit({ cid, tit, start_time, end_time, imageUrl, type }) {
    const $sql =
      "update carousel set tit=?, start_time=?, end_time=?,img=?,type=? where cid=?";
  ...
  async list({ type, pagesize, pagecount }) {
    ...
    const $sql = `select * from carousel where type=${
